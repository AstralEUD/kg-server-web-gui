...
export default function LogViewer({ serverId = "default" }: LogViewerProps) {
    ...
    const sendCommand = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`http://localhost:3000/api/servers/${serverId}/rcon`, {
                method: "POST",
    ...
    return (
        <div className="bg-zinc-800/50 rounded-lg p-4 h-[600px] flex flex-col border border-zinc-700">
            ...
            {/* Command Input */}
            <form onSubmit={sendCommand} className="flex gap-0 bg-black/40 rounded-b border border-zinc-800 border-t-0 p-2">
